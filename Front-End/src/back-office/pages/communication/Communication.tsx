      ...
      {online !== undefined && (
        <div
          ...
          style={{
            width: size * 0.3,
            height: size * 0.3,
            background: online ? '#6BB700' : '#8A8886',
          }}
        />
      )}
    </div>
...
function VideoTile({
  ...
  return (
    <div
      className="relative rounded-lg overflow-hidden bg-[#201F1E] flex items-center justify-center group"
    ...
    >
      {stream ? (
        ...
        <div className="flex flex-col items-center gap-2">
          <Av name={name} size={52} />
          <span className="text-white/60 text-xs">{name}</span>
        </div>
      )}
      {isScreen && (
        <div className="absolute top-2 left-2 bg-[#6264A7] text-white text-[10px] px-2 py-0.5 rounded font-medium">
          Presenting
        </div>
      )}
      <div
...
function NewChannelModal({
  ...
  const handleCreate = async () => {
    ...
    setLoading(true);
    try {
      await onCreate(name, type, selected);
      onClose();
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3 className="font-semibold text-base" style={{ color: '#201F1E' }}>
            Create a channel
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {(['public', 'private'] as const).map((t) => (
                <button
                  ...
                  style={{
                    border: `1px solid ${type === t ? '#6264A7' : '#E1DFDD'}`,
                    background: type === t ? 'rgba(98,100,167,0.06)' : 'white',
                    color: type === t ? '#6264A7' : '#605E5C',
                  }}
                >
                  <span>{t === 'public' ? '🌐' : '🔒'}</span>
            ...
            <p className="text-xs mt-1.5" style={{ color: '#8A8886' }}>
              {type === 'public'
                ? 'Everyone in your organization can join.'
          ...
          {type === 'private' && (
            <div>
              ...
              <div
                ...
                style={{ border: '1px solid #E1DFDD', maxHeight: 180, overflowY: 'auto' }}
              >
                {members.map((m) => (
                  <label
                    key={m.id}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate" style={{ color: '#201F1E' }}>
                        {m.name}
                      </p>
                      <p className="text-[11px] capitalize truncate" style={{ color: '#8A8886' }}>
                        {m.role.replace(/_/g, ' ')}
                      </p>
                    </div>
                  </label>
              ...
              {selected.length > 0 && (
                <p className="text-xs mt-1.5" style={{ color: '#6264A7' }}>
...
function TeamsTab({
  ...
  const roleColors: Record<string, string> = {
    'business owner': '#6264A7',
    'business admin': '#0078D4',
    accountant: '#038387',
    'team member': '#498205',
    client: '#C43E1C',
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: '#F5F5F5' }}>
      {/* Header */}
      <div
        className="sticky top-0 z-10 px-6 py-4 border-b bg-white"
        style={{ borderColor: '#E1DFDD' }}
      >
        <div className="flex items-center justify-between mb-3">
          <div>
            <h2 className="font-semibold text-base" style={{ color: '#201F1E' }}>
              Team Members
            </h2>
            <p className="text-xs" style={{ color: '#8A8886' }}>
              {members.length} members · {onlineIds.length} online
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              style={{
                background: view === 'list' ? '#6264A7' : 'transparent',
                color: view === 'list' ? 'white' : '#8A8886',
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium transition-all"
              style={{
                background: view === 'org' ? '#6264A7' : 'transparent',
                color: view === 'org' ? 'white' : '#8A8886',
                border: '1px solid #E1DFDD',
              }}
            >
              Org Chart
              ...
              <div key={role}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span
                    ...
                    style={{
                      background: `${roleColors[role] || '#6264A7'}15`,
                      color: roleColors[role] || '#6264A7',
                    }}
                  >
                ...
                <div className="grid grid-cols-1 gap-2">
                  {roleGroups[role].map((m) => {
                    ...
                    return (
                      <div
                        key={m.id}
                        className="bg-white rounded-xl px-4 py-3 flex items-center gap-4 transition-all hover:shadow-sm"
                      ...
                      >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isOnline && (
                              <span
                                ...
                                style={{ background: 'rgba(107,183,0,0.1)', color: '#6BB700' }}
                              >
                                Online
                              </span>
                            )}
                          </div>
                          <p className="text-xs mt-0.5 truncate" style={{ color: '#8A8886' }}>
                            {m.email}
                          </p>
                        </div>
                        <div className="text-right flex-shrink-0">
                          <span
                            ...
                            style={{
                              background: `${roleColors[role] || '#6264A7'}12`,
                              color: roleColors[role] || '#6264A7',
                            }}
                          >
                            {role}
...
function OrgCard({
  ...
  return (
    <div
      className="bg-white rounded-xl p-4 flex flex-col items-center gap-2 w-32 text-center transition-all hover:shadow-md"
      style={{ border: `1px solid ${color}25`, boxShadow: `0 2px 8px ${color}10` }}
    >
      <Av name={member.name} size={44} online={online} />
      <div>
        <p className="text-xs font-semibold leading-tight" style={{ color: '#201F1E' }}>
          {member.name}
        </p>
        <p className="text-[10px] mt-0.5 capitalize" style={{ color: '#8A8886' }}>
      ...
      <div className="flex items-center gap-1">
        <div
          ...
          style={{ background: online ? '#6BB700' : '#8A8886' }}
        />
        <span className="text-[9px]" style={{ color: '#8A8886' }}>
      ...
      <div className="p-6 grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        >
          ...
          >
            ...
            <h3 className="font-semibold text-sm" style={{ color: '#201F1E' }}>
              {monthName}
            </h3>
            <button
              onClick={nextMonth}
          ...
          <div className="grid grid-cols-7">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  key={day}
                  className="h-14 border-b border-r p-1 flex flex-col"
                  style={{
                    borderColor: '#F0F0F0',
                    background: isToday ? 'rgba(98,100,167,0.04)' : 'white',
                  }}
                >
                  <div
                    className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium mb-0.5 ${isToday ? 'text-white' : ''}`}
                    style={{
                      background: isToday ? '#6264A7' : 'transparent',
                      color: isToday ? 'white' : '#201F1E',
                    }}
                  >
                    {day}
                  </div>
                  {dayMeetings.slice(0, 2).map((m, mi) => (
                    <div
                      ...
                      style={{
                        background:
                          m.type === 'video' ? 'rgba(98,100,167,0.15)' : 'rgba(3,131,135,0.15)',
                        color: m.type === 'video' ? '#6264A7' : '#038387',
                      }}
                    >
              ...
              <div className="divide-y" style={{ borderColor: '#F0F0F0' }}>
                {upcomingMeetings.map((m) => (
                  <div key={m.id} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium" style={{ color: '#201F1E' }}>
                        #{m.channelName}
                      </p>
                      <p className="text-[11px]" style={{ color: '#8A8886' }}>
                        Started {fmtTime(m.startedAt)} · {m.participants.length} participant
              ...
              <div className="divide-y" style={{ borderColor: '#F0F0F0' }}>
                {pastMeetings.map((m) => (
                  <div key={m.id} className="px-5 py-3 flex items-center gap-3">
                    ...
                    <div className="text-right flex-shrink-0">
                      {m.duration && (
                        <p className="text-xs font-medium" style={{ color: '#6264A7' }}>
...
export function Communication() {
  ...
  useEffect(() => {
    ...
    socket.on('user:online', ({ userId }: any) =>
      setOnlineIds((p) => [...new Set([...p, userId])])
    );
    ...
    socket.on('call:peer-joined', async ({ userId, userName }: any) => {
      ...
      setMeetings((prev) =>
        prev.map((m) =>
          m.callId === curCallId.current
            ? { ...m, participants: [...new Set([...m.participants, userName])] }
            : m
        )
      );
    });
    ...
    socket.on('webrtc:ice', async ({ from, candidate }: any) => {
      await pcs.current.get(from)?.addIceCandidate(candidate);
    });
  ...
  const mkPC = useCallback((userId: string, userName: string, socket: Socket) => {
    ...
    pc.ontrack = (e) =>
      setPeers((p) => {
        const n = new Map(p);
        n.set(userId, { userId, userName, stream: e.streams[0] });
        return n;
      });
    pc.onicecandidate = (e) => {
      if (e.candidate)
    ...
    setPeers((p) => {
      const n = new Map(p);
      if (!n.has(userId)) n.set(userId, { userId, userName });
      return n;
    });
    return pc;
  }, []);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/communication/channels`, { headers })
      .then((r) => r.json())
      .then((data) => {
        if (Array.isArray(data)) {
          setChannels(data);
          if (data.length > 0) joinChannel(data[0]);
        }
      });
  ...
  const joinChannel = useCallback((ch: Channel) => {
    ...
    const h = (msgs: Message[]) => {
      setMessages(msgs);
      socketRef.current?.off('channel:history', h);
    };
  ...
  const acceptCall = useCallback(async () => {
    ...
    curCallId.current = incomingCall.callId;
    setInCall(true);
    callStartTime.current = new Date();
  ...
  const endCall = useCallback(() => {
    ...
    setMeetings((prev) =>
      prev.map((m) =>
        m.callId === curCallId.current ? { ...m, endedAt: new Date().toISOString(), duration } : m
      )
    );
    curCallId.current = null;
    setSharing(false);
    setCallDuration(0);
  }, []);
  ...
  const grouped = messages.reduce<(Message & { grouped: boolean; dateSep?: string })[]>(
    (acc, msg, i) => {
  ...
  return (
    <>
      ...
      <div
        className="ms-root flex h-[calc(100vh-5rem)] rounded-xl overflow-hidden bg-white"
      ...
      >
        ...
        >
          <div className="w-9 h-9 rounded-lg bg-white/20 flex items-center justify-center text-white font-bold text-sm mb-3">
            BM
          </div>
          {navItems.map((n) => (
            <button
        ...
        {activeNav === 'chat' && (
          <div
            ...
            style={{ background: '#F5F5F5', borderColor: '#E1DFDD' }}
          >
            <div className="px-4 py-3 border-b" style={{ borderColor: '#E1DFDD' }}>
              <div className="flex items-center justify-between mb-2">
                <h2 className="font-semibold text-sm" style={{ color: '#201F1E' }}>
                  Chat
                </h2>
                {isAdmin && (
                  <button
            ...
            <div className="flex-1 overflow-y-auto ms-scroll py-2">
              {isAdmin && (
                <div
                  className="mx-3 mb-2 px-3 py-1.5 rounded-lg flex items-center gap-2"
                  style={{
                ...
                >
                  ...
                  <span className="text-[11px] font-semibold" style={{ color: '#6264A7' }}>
                    Admin
                  </span>
                </div>
              )}
              ...
              {channels.map((ch) => (
                ...
                >
                  <div
                    ...
                    style={{
                      background: activeChannel?.id === ch.id ? 'rgba(98,100,167,0.2)' : '#E8E8E8',
                      color: activeChannel?.id === ch.id ? '#6264A7' : '#8A8886',
                    }}
                  >
                    {ch.type === 'private' ? '🔒' : '#'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p
                      ...
                      style={{ color: activeChannel?.id === ch.id ? '#6264A7' : '#201F1E' }}
                    >
                      {ch.name}
                    </p>
                  </div>
              ...
              <p
                className="px-4 py-1 mt-3 text-[10px] font-semibold uppercase tracking-widest"
                style={{ color: '#8A8886' }}
              >
              ...
              {onlineIds.slice(0, 8).map((id) => (
                <div
                  key={id}
                  className="chan-item flex items-center gap-2.5 px-4 py-1.5 cursor-default"
                >
                  <Av
                    name={id === user?.id ? (user?.name ?? 'U') : 'User'}
                    size={28}
                    online={true}
                  />
            ...
            <div className="p-3 border-t" style={{ borderColor: '#E1DFDD' }}>
              <div className="flex items-center gap-2.5 px-2 py-2 rounded-xl chan-item">
                <Av name={user?.name ?? 'U'} size={32} online={true} />
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: '#201F1E' }}>
                    {user?.name}
                  </p>
                  <p className="text-[10px] capitalize truncate" style={{ color: '#8A8886' }}>
        ...
        {activeNav === 'calendar' && (
          <CalendarTab
            ...
            onStartMeeting={() => {
              setActiveNav('chat');
              setTimeout(() => startCall('video'), 100);
            }}
          />
        )}
          ...
          <div className="flex-1 flex flex-col min-w-0 bg-white">
            ...
            >
              <div className="flex items-center gap-3">
                {activeChannel ? (
                  ...
                  <span className="text-sm" style={{ color: '#8A8886' }}>
                    Select a channel
                  </span>
                )}
              </div>
            ...
            {inCall && (
              <div className="flex-shrink-0 flex flex-col" style={{ background: '#201F1E' }}>
                <div className="p-4">
                  <div
                    className={`grid gap-3 ${peersArr.length === 0 ? 'grid-cols-1 max-w-sm mx-auto' : peersArr.length === 1 ? 'grid-cols-2' : peersArr.length <= 3 ? 'grid-cols-3' : 'grid-cols-4'}`}
                  >
                    <VideoTile
                      stream={sharing && screenRef.current ? screenRef.current : localStream}
                      name={user?.name ?? 'You'}
                      muted
                      isScreen={sharing}
                    />
                    {peersArr.map((p) => (
                      <VideoTile key={p.userId} stream={p.stream ?? null} name={p.userName} />
                    ))}
                  </div>
                  {peersArr.length === 0 && (
                ...
                return (
                  <div key={msg.id}>
                    {msg.dateSep && (
                      <div className="flex items-center gap-3 my-4">
                        ...
                        <span className="text-[11px] font-medium px-2" style={{ color: '#8A8886' }}>
                          {new Date(msg.dateSep).toLocaleDateString([], {
                            weekday: 'long',
                            month: 'long',
                            day: 'numeric',
                          })}
                    ...
                    <div className={`msg-in flex gap-3 ${msg.grouped ? 'mt-0.5' : 'mt-4'} group`}>
                      {!msg.grouped ? (
                        <Av name={msg.senderName} size={36} />
                      ) : (
                      ...
                      <div className="flex-1 min-w-0">
                        {!msg.grouped && (
                          <div className="flex items-baseline gap-2 mb-0.5">
                            <span className="font-semibold text-sm" style={{ color: '#201F1E' }}>
                              {isMe ? 'You' : msg.senderName}
                            </span>
                            <span className="text-[11px]" style={{ color: '#8A8886' }}>
                              {fmtTime(msg.createdAt)}
                            </span>
                          </div>
                        )}
                        <p className="text-sm leading-relaxed" style={{ color: '#201F1E' }}>
                          {msg.content}
                        </p>
                      </div>
                    </div>
            ...
            <div className="px-4 pb-4 pt-2 flex-shrink-0">
              <div
                ...
                style={{ border: '1px solid #E1DFDD', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}
              >
                <div
                ...
                >
                  {['B', 'I', 'U'].map((f) => (
                    <button
                      key={f}
                      className="w-6 h-6 rounded text-xs font-bold text-gray-500 hover:bg-gray-200 transition-colors"
                    >
                      {f}
                    </button>
                  ))}
                  ...
                  {['😊', '📎', '@'].map((ic) => (
                    <button
                      key={ic}
                      className="w-6 h-6 rounded text-sm text-gray-500 hover:bg-gray-200 flex items-center justify-center"
                    >
                      {ic}
                    </button>
                  ))}
              ...
              <p className="text-[10px] mt-1.5 ml-1" style={{ color: '#8A8886' }}>
        ...
        >
          ...
          >
            <div className="px-5 py-4">
              <p className="text-white/60 text-xs mb-3 font-medium uppercase tracking-wider">
